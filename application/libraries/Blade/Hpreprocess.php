...
class HPreProcess{
	private $CI;
	private static $token_name = 'enuy_csrf_name';

  	private static $token;
	public function __construct()
	{
		$this->CI =& get_instance();
		 $this->CI->load->helper(array('enuy'));
	}
	private function addAnalytic($output){
		if(strpos(strtolower(uri_string()), "admin") !==FALSE){
			return $output;
		}
		else{
			$ana = "<script>";
			...
			$ana .="</script>";
			$output = preg_replace("/<\/head>/",ENUY_HEADER().$ana."$0", $output);
			...
			$output = preg_replace("/<\/body>/", ENUY_FOOTER().$editthtml."$0", $output);
			return $output;
		}
	}
	...
	public function replaceHtml($output){
