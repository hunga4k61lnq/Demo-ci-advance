...
<div class="col-md-4 col-xs-12 itemsearch">
	...
	<input value="<?php echo !isNull($gtfrom)? date('d-m-Y H:i:s',$gtfrom):'' ?>" class="datepicker" value="" type="text" placeholder="Từ (<?php echo $value['note'] ?>)" onkeydown="">
	<input value="<?php echo !isNull($gtfrom)?$gtfrom:'' ?>" type="hidden" name="search_<?php echo $value['name']?>_from">
	<span>=></span>
	<input value="<?php echo !isNull($gtto)?date('d-m-Y H:i:s',$gtto):'' ?>" class="datepicker" value=""  type="text" placeholder="Đến (<?php echo $value['note'] ?>)" onkeydown="">
