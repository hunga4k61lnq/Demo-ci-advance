var imageEditNameGlobal =undefined;
var featherEditor = new Aviary.Feather({
    ...
    onSave: function(imageID, newURL) {
        ...
        img.src = newURL;
         featherEditor.close();
        $.ajax({
          url: 'Admin/pfDownloadImage',
          type: 'POST',
          data: {file: newURL,name:imageEditNameGlobal},
        })
        .done(function(e) {
          ...
          try{
            ...
            bootbox.alert("Download thất bại!");  
          }
        })
        .fail(function() {
          console.log("error");
...
function launchEditor(id, src,imageEditName) {
    ...
    imageEditNameGlobal = imageEditName;
    return false;
}
/* CLEAR*/
...
.          css({
  ...
  $('i.download').click(function(event) {
            ...
            bootbox.confirm("Bạn có muốn download "+name+" ?",function(e){
              ...
              if(e){
                window.location.href=globalBaseUrl+"Admin/pfForceDownload?name="+name;
      ...
      if(!objfile.hasOwnProperty('isfile') || objfile.isfile ==0){
        ...
        if(existFolder.trim().length==0){
          ...
          if(href.indexOf('?')!=-1){
            ...
            window.location.href=globalBaseUrl+"Admin/mediaManager?folder="+objfile.name;
          }
        }
        else{
...
{
  var uploadURL ="Admin/uploadMultiFile"; //Upload URL
